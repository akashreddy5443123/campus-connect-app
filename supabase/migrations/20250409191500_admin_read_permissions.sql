...
-- Update RLS policies to allow admins to perform all operations
DROP POLICY IF EXISTS "Allow admin all access to events" ON public.events;
CREATE POLICY "Allow admin all access to events"
ON public.events
TO authenticated
USING (
  EXISTS (
...
-- Update your user to be an admin (replace with your actual user ID)
